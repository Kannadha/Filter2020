...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("🔙 𝙱𝙰𝙲𝙺 ", callback_data=f"pnext_{req}_{key}_{off_set}"), InlineKeyboardButton(f"📃 Pages {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")]
        )
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"📄 1/{round(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="𝙽𝙴𝚇𝚃 ➡️",callback_data=f"pnext_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
